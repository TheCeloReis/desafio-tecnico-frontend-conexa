...
function useForm({ initialState }: UseFormArgs = {}) {
  ...
  const setFormState = useCallback((newState: Record<string, string>) => {
    setState(transformInitialState(newState));
  }, []);
  ...
  const register = (field: string, options: RegisterOptions = {}) => {
    ...
    return {
      ...
      isSelected: state[field]?.value === "checked" ? true : false,

      errorMessage: state[field]?.error,
    };
  };
  ...
  return { formState, isValid, register, validate, setField, setFormState };
}
